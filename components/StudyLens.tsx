...
const analyzeImage = async (cropPct: any) => {
    ...
    try {
      ...
      if (compressed) {
        sendB64 = compressed.b64;
        const thumbCompressed = await compressDataUrl(compressed.dataUrl, 500_000);
        thumbnail = thumbCompressed ? thumbCompressed.dataUrl : compressed.dataUrl;
      } else {
        sendB64 = croppedDataUrl.split(",")[1] || "";
        thumbnail = croppedDataUrl;
        if (sendB64.length > 4_800_000) { setErr("Fotoğraf çok büyük."); setLoading(false); return; }
      }
      raw = await callClaude([{ role: "user", content: [{ type: "image", source: { type: "base64", media_type: "image/jpeg", data: sendB64 } }, { type: "text", text: `Analyze ALL educational questions visible in this image. Return ONLY valid JSON, start with { end with }, no backticks: {"questions":[{"subject":"Matematik","topic":"topic in Turkish","subtopic":"subtopic in Turkish","difficulty":"Kolay or Orta or Zor","question":"full question text in Turkish","summary":"one sentence in Turkish","answer":"Bu soruyu adım adım çöz. Eğer soru grafik, şekil veya koordinat düzlemi içeriyorsa kesin sayısal cevap verme, sadece hangi özelliklere bakılması gerektiğini ve çözüm yöntemini açıkla. Grafik veya şekil içermeyen düz hesap sorularında ise adımları açıkla ve son adımda kesin cevabı yaz.","advice":"one sentence in Turkish"}]}` }] }], 8000);
