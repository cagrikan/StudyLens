...
const analyzeImage = async (cropPct: any) => {
    ...
    try {
      ...
      raw = await callClaude([{ role: "user", content: [{ type: "image", source: { type: "base64", media_type: "image/jpeg", data: sendB64 } }, { type: "text", text: `Analyze ALL educational questions visible in this image. Return ONLY valid JSON, start with { end with }, no backticks: {"questions":[{"subject":"Matematik","topic":"topic in Turkish","subtopic":"subtopic in Turkish","difficulty":"Kolay or Orta or Zor","question":"full question text in Turkish","summary":"one sentence in Turkish","answer":"Bu soruyu adım adım çöz. Eğer soru şekil veya grafik içeriyorsa kesin cevap verme sadece çözüm yöntemini açıkla. Diğer sorularda adımları açıkla ve son adımda kesin cevabı belirt.","advice":"one sentence in Turkish"}]}` }] }], 8000);
